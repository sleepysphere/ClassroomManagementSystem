...
public class SchedulePanel extends JPanel {
    ...
    private JPanel createTablePanel() {
        ...
        String[] columns = {"Class ID", "Course ID", "Instructor ID", "Room ID", "Date", "Start Time", "End Time", "Semester"};
    ...
    private void showAddScheduleDialog() {
        ...
        JLabel courseIdLabel = new JLabel("Course:");
        courseIdLabel.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        JComboBox<String> courseCombo = new JComboBox<>();
        courseCombo.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        for (model.Course c : repository.sql.CourseRepositorySQL.getAllCourses()) {
            courseCombo.addItem(c.getCourseId() + " - " + c.getCourseName());
        }

        JLabel instructorIdLabel = new JLabel("Instructor:");
        instructorIdLabel.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        JComboBox<String> instructorCombo = new JComboBox<>();
        instructorCombo.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        for (model.Instructor i : repository.sql.InstructorRepositorySQL.getAllInstructors()) {
            instructorCombo.addItem(i.getInstructorId() + " - " + i.getFirstName() + " " + i.getLastName());
        }

        JLabel roomIdLabel = new JLabel("Room:");
        roomIdLabel.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        JComboBox<String> roomCombo = new JComboBox<>();
        roomCombo.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        for (model.Room r : repository.sql.RoomRepositorySQL.getAllRooms()) {
            roomCombo.addItem(r.getRoomID() + " - " + r.getRoomNumber());
        }
        
        JLabel sessionTypeLabel = new JLabel("Room Type:");
        sessionTypeLabel.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        java.util.Set<String> roomTypes = new java.util.HashSet<>();
        for (model.Room r : repository.sql.RoomRepositorySQL.getAllRooms()) {
            if (r.getRoomType() != null && !r.getRoomType().isEmpty()) {
                roomTypes.add(r.getRoomType());
            }
        }
        JComboBox<String> sessionTypeCombo = new JComboBox<>(roomTypes.toArray(new String[0]));
        ...
        datePanel.setBackground(MainFrame.PANEL_COLOR);

        SpinnerNumberModel yearModel = new SpinnerNumberModel(2024, 2020, 2030, 1);
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (courseCombo.getSelectedIndex() == -1 || instructorCombo.getSelectedIndex() == -1 || roomCombo.getSelectedIndex() == -1 || semester.isEmpty()) {
            ...
            try {
                ...
                int instructorId = Integer.parseInt(instructorCombo.getSelectedItem().toString().split(" - ")[0]);
    ...
    private void refreshTable() {
        ...
        for (Schedule schedule : ScheduleRepositorySQL.getAllSchedules()) {
            Object[] row = {
                ...
                schedule.getRoomId(),
                schedule.getStartDate().format(java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy")),
